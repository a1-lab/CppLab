...
void wxCheckTree::Init()
{
    ...
    Bind(wxEVT_TREE_STATE_IMAGE_CLICK, &wxCheckTree::OnItemStateClick, this);
}
...
void wxCheckTree::OnItemStateClick(wxTreeEvent& event)
{
    wxTreeItemId m_lastId = event.GetItem();

    int state = GetItemState(m_lastId);

    if (state == wxCHECKTREE_STATE_UNCHECKED || 
        state == wxCHECKTREE_STATE_UNCHECKED_CURRENT ||
        state == wxCHECKTREE_STATE_UNCHECKED_PRESSED) {
        SetItemState(m_lastId, wxCHECKTREE_STATE_CHECKED);
        return;
    } 
    ...
    /*
        ...
        state == wxCHECKTREE_STATE_CHECKED_CURRENT) {
        SetItemState(m_lastId, wxCHECKTREE_STATE_UNCHECKED);
    }*/
